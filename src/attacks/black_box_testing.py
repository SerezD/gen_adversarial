...
class CoupledDataset(Dataset):
    def __init__(self, opt, train: bool = True):

        data_dir = f'{opt.data_dir}/cifar-10-batches-py'
        ...
        if train:
            ...
            for i in range(1, 6):
                with open(f'{data_dir}/data_batch_{i}', 'rb') as fo:
                    train_set = pickle.load(fo, encoding='bytes')
                    for l in train_set[b'labels']:
                        img_labels.append(l)
                    imgs = np.concatenate([imgs, train_set[b'data']], axis=0)
            ...
            self.img_labels = torch.tensor(test_set[b'labels'])
            self.imgs = torch.tensor(test_set[b'data']).reshape(-1, 3, 32, 32)
    ...
    def __getitem__(self, idx):
        ...
        trg_idx = torch.randint(len(cand_trg), (1,)).item()
        trg_y = self.img_labels[trg_idx]

        src_x = self.imgs[idx]
        trg_x = self.imgs[trg_idx]
        return src_x, src_y, trg_x, trg_y


def init_run():
    parser = argparse.ArgumentParser('train a non linear latent walker for a black box attack setting.')
    ...
    parser.add_argument('--run_name', type=str, default='blackbox_walker')
...
def prepare_data(opt):
    train_dataset = CoupledDataset(opt, train=True)
    ...
    test_dataset = CoupledDataset(opt, train=False)
...
def main(rank, world_size, opt):
    ...
    for epoch in range(opt.n_epochs):
        ...
        for batch_index, batch in enumerate(tqdm(train_dataloader)):
            ...
            src_x, src_y, trg_x, trg_y = [x.to(opt.rank) for x in batch]
            b, _, _, _ = src_x.shape

            with torch.no_grad():
                src_clean_y = torch.argmax(attacked_cnn.module(preprocess(src_x)), dim=1)
                trg_clean_y = torch.argmax(attacked_cnn.module(preprocess(trg_x)), dim=1)
                src_chunks = nvae.module.encode(src_x)
                trg_chunks = nvae.module.encode(trg_x)

            optimizer.zero_grad()

            # get adversarial chunks
            adversarial_trg_chunks = []
            for i, c in enumerate(trg_chunks):
                adv_c = walker.module(c, i)
                r = int(math.sqrt(adv_c.shape[1] // nvae.module.num_latent_per_group))
                adversarial_trg_chunks.append(adv_c.view(b, nvae.module.num_latent_per_group, r, r))

            with torch.no_grad():
                # decode everything and obtain preds.
                nvae_logits = nvae.module.decode(adversarial_chunks)
                adversarial_samples = nvae.module.decoder_output(nvae_logits).sample()
                trg_adv_y = torch.argmax(attacked_cnn.module(adversarial_samples), dim=1)
                what_to_minimize = torch.equal(trg_y, trg_adv_y)

            # loss on valid chunks (these that did not change class)
            loss = None
        ...
        with torch.no_grad():
            ...
            for batch_index, batch in enumerate(tqdm(test_dataloader)):
                ...
                if opt.rank == 0 and batch_index == 0:
                    ...
                    plt.savefig(f'{opt.plots_folder}/samples_ep{epoch}.png')
                    plt.close()
            ...
            if opt.rank == 0:
                ...
                if adv_acc_atk < best_accuracy:
                    ...
                    # saving model
                    torch.save(walker.state_dict(), f'{opt.ckpt_folder}/ep_{epoch}')
