...
device = 'cuda' if torch.cuda.is_available() else 'cpu'   # TODO check device
...
def kl_balancer(kl_unbalanced_terms: list, beta: float = 1.0, balance: bool = False,
    ...
    if balance and beta < 1.0:
        ...
        kl_all = torch.stack(kl_unbalanced_terms, dim=1)
    ...
    return beta * kl, kl_coeffs, kl_vals
...
def kl_balancer_coeff(num_scales, groups_per_scale, fun):
    ...
    if fun == 'equal':
        coeff = torch.cat([torch.ones(groups_list[num_scales - i - 1]) for i in range(num_scales)], dim=0).to(device)
    elif fun == 'linear':
        coeff = torch.cat([(2 ** i) * torch.ones(groups_list[num_scales - i - 1]) for i in range(num_scales)], dim=0).to(device)
    elif fun == 'sqrt':
        coeff = torch.cat([np.sqrt(2 ** i) * torch.ones(groups_list[num_scales - i - 1]) for i in range(num_scales)], dim=0).to(device)
    elif fun == 'square':
        coeff = torch.cat([np.square(2 ** i) / groups_list[num_scales - i - 1] * torch.ones(groups_list[num_scales - i - 1]) for i in range(num_scales)], dim=0).to(device)
    else:
