...
def main():
    ...
    for b in tqdm(range(0, data_n, BATCH_SIZE)):
        ...
        _, normalized_chunks_cln = nvae.encode(x.to('cuda:0'))
        _, normalized_chunks_adv = nvae.encode(x_adv.to('cuda:0'))

        # l2 distance CLN ADV
        chunks_l2 = torch.empty((BATCH_SIZE, 0), device='cuda:1')
        for (c, a) in zip(normalized_chunks_cln, normalized_chunks_adv):

            chunks_l2, _ = pack([
                chunks_l2,
                (c - a).view(BATCH_SIZE, -1).pow(2).mean(dim=1).sqrt().unsqueeze(1).to('cuda:1')
            ] , 'b *')
    ...
    #         _, chunks_adv, _, _, _, _ = nvae.encode(x_adv.to('cuda:0'))
    #         x_def = nvae.decoder_output(nvae.sample_from_chunk0(chunks_adv[:2], t=1.)).sample()
    #         x_def_tmp, _ = pack([x_def_tmp, x_def.to('cuda:1').unsqueeze(1)], 'b * c h w')
    #
    #         # encode adv and get chunks
    #         _, chunks_cln, _, _, _, _ = nvae.encode(x.to('cuda:0'))
    #         x_cln = nvae.decoder_output(nvae.sample_from_chunk0(chunks_cln[:2], t=1.)).sample()
...
if __name__ == '__main__':
    ...
    CKPT_NVAE = '/media/dserez/runs/NVAE/cifar10/best/checkpoint.pt'
